...
import { fields, styles } from '@/js/modules/settings/feature-access/basic'
...
const onHandleEdit = (item) => {
  ...
  router.push({ name: 'settings.features.edit', params: { slug: data } })
}

const onHandleDelete = (item) => {
  _confirm(
    { title: 'Hapus Akses Fitur', text: `Apakah Anda yakin untuk menghapus fitur: ${ item.nama_fitur }`, icon: 'question' },
    () => {
      _http.delete(
        _route('backend.setting.feature.delete'),
    ...
    .then(({ value, isConfirmed, isDismissed }) => {
      ...
      if (isConfirmed && value)
        _alert.fire({ title: `Hapus Data Berhasil`, text: `Fitur ${ item.nama_fitur } berhasil dihapus`, 'icon': 'success' })
  ...
  <div class="panel">
    <div class="panel-hdr">
      <h2 class="fs-xl">
        ...
        <strong class="fw-600">
          Daftar&nbsp;<span class="fw-300">Akses Fitur</span>
        </strong>
      </h2>
    </div>
      ...
      <div class="panel-content">
        <div class="action-wrapper d-flex flex-column flex-md-row align-items-baseline justify-content-md-between mb-2">
          <b-button
            ...
            @click="$router.push({ name: 'settings.features.add' })"
          >
