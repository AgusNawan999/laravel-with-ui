...
import { _http, _route, _alert, _encrypt, _confirm } from "@/js/utils/common";
import { fields, styles } from "@/js/modules/settings/management-users/basic.js";
...
const loadProvider = _.debounce((ctx, callback) => {
  ...
  _http
    .get(
      _route("backend.setting.management.user.get", {
...
const onHandleEdit = (item) => {
  ...
  router.push({ name: 'settings.management.users.edit', params: { slug: data } })
}
...
_confirm(
    { title: 'Hapus Pengguna', text: `Apakah Anda yakin untuk menghapus pengguna: ${ item.item.v_username }`, icon: 'question' },
    () => {
      _http.delete(
        _route('backend.setting.management.users.delete'),
        { params: { '_method': 'delete', slug: item.item.slug } }
      )
        .then(result => result)
    ...
    .then(({ value, isConfirmed, isDismissed }) => {
      ...
      if (isConfirmed && value)
        _alert.fire({ title: `Hapus Data Berhasil`, text: `pengguna ${ item.item.v_username } berhasil dihapus`, 'icon': 'success' })
  ...
  <div class="panel">
    <div class="panel-hdr">
      <h2 class="fs-xl">
        ...
        <strong class="fw-600">
          Daftar&nbsp;<span class="fw-300">Managemen Pengguna</span>
        </strong>
      </h2>
    </div>
    <div class="panel-container">
      <div class="panel-content">
        <div
          class="action-wrapper d-flex flex-column flex-md-row align-items-baseline justify-content-md-between mb-2"
        >
          <b-button
            variant="primary"
            class="btn-sm-block mb-2 mb-md-0"
            @click="$router.push({ name: 'settings.management.users.add' })"
          >
            <i class="fad fa-plus-circle"></i>&nbsp; Tambah
          </b-button>
          <div class="searching-wrapper flex-1">
            <div class="d-flex justify-content-md-end align-items-center">
              <b-button variant="info" class="btn-sm-block mr-0 mr-sm-2 mb-2 mb-sm-0" @click.prevent="refTable.refresh()">
                <i class="fad fa-sync"></i>&nbsp; Muat Ulang
              </b-button>
              <div
                :class="{
                  'd-flex position-relative align-items-center': true,
                  wider: isFocus,
                }"
              >
        ...
        >
          ...
          <template #cell(action)="row">
            <div class="d-flex align-items-baseline">
              <b-button variant="link" class="text-info py-0 px-1" @click.prevent="onHandleEdit(row)">
                <i class="fad fa-edit"></i>
              </b-button>
              <b-button variant="link" class="text-danger py-0 px-1" @click.prevent="onHandleDelete(row)">
          ...
          <template #cell(si_user_enable)="row">
            <span> {{ row.item.si_user_enable == 1 ? 'Aktif' : 'Tidak Aktif' }} </span>
          </template>
          <template #cell(group)="row">
            <span v-if="row.item.user_group == 0"></span>
            <div  v-if="row.item.user_group != 0">
              <span
                v-for="(items, key) in row.item.user_group" :key="key"
                :class="{
                  'badge border border-primary text-black': true,
                  'mr-1': key != (items.detail_group.v_group_name - 1)
                }"
              >{{ items.detail_group.v_group_name }}</span>
            </div>
          </template>
        </b-table>
        <div class="d-flex justify-content-between align-items-center">
          <b-dropdown
            size="sm"
            variant="default"
            class="mr-2 form-control-height-sm"
